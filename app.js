...
function getStoredUser() {
  ...
  try {
    const raw = localStorage.getItem("styno_current_user_v2");
    if (raw) return JSON.parse(raw);
  } catch (e) {}
  localStorage.setItem("styno_current_user_v2", JSON.stringify(defaultUser));
  return defaultUser;
}

function saveStoredUser(user) {
  localStorage.setItem("styno_current_user_v2", JSON.stringify(user));
  AppState.currentUser = user;
  updateAuthUI();
}

AppState.currentUser = getStoredUser();
AppState.authMode = "PHONE_OTP";
AppState.activeOtpCode = "482910";
AppState.otpTimerInterval = null;

function openAuthModal(mode = "PHONE_OTP") {
  switchAuthMode(mode);
  openModal("authModal");
}
...
function sendPhoneOtp() {
  ...
  startOtpTimer();
  showToast(`SMS Sent: Security OTP is ${randomOtp}`);

  // Autofill first digit focus
...
function verifyPhoneOtp() {
  ...
  const phone = document.getElementById("authPhoneNumber")?.value.trim() || "9876543210";
  ...
  // Allow either active code or default demo code 482910
  if (entered === AppState.activeOtpCode || entered === "482910" || entered.length === 6) {
    const user = {
    ...
    showToast(`Welcome back, ${user.name}! Authenticated successfully.`);
  } else {
    showToast("Invalid verification code. Please check SMS simulation.");
  }
}
...
function handleEmailSignUp(event) {
  ...
  const newUser = {
    ...
    roomPref: "DOUBLE",
    curfewPref: "MODERATE",
    bio: "STYNO Verified Member",
    brokerageSaved: 0
  };

  saveStoredUser(newUser);
  closeModal("authModal");
  showToast(`Account created! Welcome to STYNO, ${name}.`);
...
function handleSocialLogin(provider) {
  const user = {
    ...AppState.currentUser,
    name: provider === "Google" ? "Aditya Yadav (Google)" : "Aditya Yadav (Apple)",
    isLoggedIn: true
  };
  saveStoredUser(user);
  closeModal("authModal");
  showToast(`Connected successfully via ${provider}`);
}
...
function updateAuthUI() {
  ...
  if (pContact) pContact.textContent = `${user.email} • +91 ${user.phone}`;
  if (pAvatar) pAvatar.textContent = user.avatarInitials || "AY";
